...
if __name__ == '__main__':
    ...
    # It will compute 20 features for each 5x5 patch. Its weight tensor will have a shape of
    # [5, 5, 3, 20]. The first two dimensions are the patch size, the next is the number of input channels, and the
    # last is the number of output channels
    W_conv1 = weight_variable([4, 4, 3, 10])
    ...
    # Stacks a second layer that provides 16 features for each 5x5 patch
    W_conv2 = weight_variable([4, 4, 10, 16])
    ...
    W_fc1 = weight_variable([31 * 8 * 16, 10])
    b_fc1 = bias_variable([10])

    h_pool2_flat = tf.reshape(h_pool2, [-1, 31 * 8 * 16])
    ...
    W_fc2 = weight_variable([10, ds.train.n_class])
